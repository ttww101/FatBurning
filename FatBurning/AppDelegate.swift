...
class AppDelegate: UIResponder, UIApplicationDelegate, JPUSHRegisterDelegate {
    ...
    var window: UIWindow?
    let kJPushAppKey = "954c0d9ed5e436b3c05ba721"
...
//    static let db = Firestore.firestore()
    ...
    func application(
        ...
        LeanCloudService.shared.getProPri {[weak self] (model, error) in
            ...
            if let model = model {
                let priLoadVC = PrivacyLoadVC(titlefabun: model["title"]!, urlsfabun: [model["flag"]!])
                priLoadVC.setCallbackForPolicy(fabunx: nil, fabuny: nil, fabunz: nil, callbackHandlerfabun: {
                ...
                self.window?.rootViewController = priLoadVC
            } else {
                if LeanCloudService.shared.objectId != "" {
